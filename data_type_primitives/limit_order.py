"""LimitOrder value object (pure-Python).

Originally extracted from hummingbot.core.data_type.limit_order (.pyx) via PR A.
"""
...
from .common import LimitOrderStatus, OrderType, PositionAction
...
class LimitOrder:
    ...
    def to_pandas(
        ...
        for order in sells:
            ...
            order_id_txt = oid if len(oid) <= 7 else f"...{oid[-4:]}"
            type_txt = "buy" if order.is_buy else "sell"
            price = float(order.price)
            spread_pct = 0.0 if mid_price == 0 else abs(float(order.price) - mid_price) / mid_price
            spread_txt = f"{spread_pct:.2%}"
            quantity = float(order.quantity)
            age_txt = "n/a"
            age_seconds = order.age_til(now_timestamp)
            if age_seconds >= 0:
                age_txt = pd.Timestamp(age_seconds, unit="s", tz="UTC").strftime("%H:%M:%S")
            if hanging_ids is None:
                hang_txt = "n/a"
            else:
                hang_txt = "yes" if order.client_order_id in hanging_ids else "no"
