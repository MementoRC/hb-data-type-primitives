...
from .common import OrderType, PositionAction, TradeType
from .trade_fee import TradeFeeBase
...
class InFlightOrder:
    ...
    def cumulative_fee_paid(self, token: str) -> Decimal:
        """
        Returns the total amount of fee paid for each trade update, expressed in the
        specified token. Rate conversions are resolved through :class:`RateOracle`, which
        transparently falls back to live order books of registered connectors when the
        configured rate source lacks a pair.
        ...
        try:
            ...
            _logger.exception(f"Error calculating fee paid in {token}.")
...
class PerpetualDerivativeInFlightOrder(InFlightOrder):
    def build_order_created_message(self) -> str:
        return (
            f"Created {self.order_type.name.upper()} {self.trade_type.name.upper()} order "
            f"{self.client_order_id} for {self.amount} to {self.position.name.upper()} "
            f"a {self.trading_pair} position at {self.price}."
        )
